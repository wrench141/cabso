...
export default function Cars(){
    ...
    const {totalDistance, markerDetails, totalTime, pricePerKm} = useSelector((state) => state.mapReducer);
    ...
    const filterCars = (brand, type) => {
        if(brand === 'All Cars' && type !== 'All'){
            ...
            setFilteredCars(allCars.filter((car) => car.brand === brand && car.type === type))
        }
    }
    ...
    return(
        <div className='carsContainer'>
            ...
            <div className='container'>
                {
                    filteredCars.map((car, i) => (
                        <Car details={car} key={i} setState={setShowBar} setSelectedBrand={setSelectedBrand}/>
                    ))
                }
            </div>
            {
                markerDetails !== undefined ? (
                    <div className='info'>
                        ...
                        <div className='extraDetails'>
                            <div className='extra'>
                                <p className='value'>{totalDistance} <span style={{fontSize:11, fontWeight:400}}>Km</span> </p>
                            ...
                            <div className='extra'>
                                <p className='value'>{EstimatedPrice} <span style={{fontSize:11, fontWeight:400}}>/-</span> </p>
                                <p className='subHead'>Average Price</p>
                            </div>
                            <div className='extra'>
                                <p className='value'>{(estTime).toFixed(2)} <span style={{fontSize:11, fontWeight:400}}>{label}</span> </p>
            ...
            {
                showBar ? (
                    <RightBar state={showBar} setState={setShowBar} brand={selectedBrand} estimatedPrice={EstimatedPrice}/>
                ) : null
            }
        </div>
    )
